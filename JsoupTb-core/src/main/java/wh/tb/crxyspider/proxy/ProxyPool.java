package wh.tb.crxyspider.proxy;

import org.apache.http.HttpHost;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import wh.tb.crxyspider.utils.FilePersistentBase;
import wh.tb.crxyspider.utils.ProxyUtils;
...
public class ProxyPool {
	...
	private String proxyFilePath = "/data/wh.tb.crxyspider/lastUse.proxy";
	...
	private void setFilePath() {
		...
		String path = tmpDir + FilePersistentBase.PATH_SEPERATOR + "wh.tb.crxyspider" + FilePersistentBase.PATH_SEPERATOR + "lastUse.proxy";
		if (tmpDir != null && new File(tmpDir).isDirectory()) {
			fBase.setPath(tmpDir + FilePersistentBase.PATH_SEPERATOR + "wh.tb.crxyspider");
	...
	private Map<String, Proxy> prepareForSaving() {
		...
		for (Map.Entry<String, Proxy> e : allProxy.entrySet()) {
			Proxy p = e.getValue();
			p.setFailedNum(0);
			tmp.put(e.getKey(), p);
		}
